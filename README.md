...
2. Install the dependencies:
    ...
    **Before running the following command there are somethings to note:**
    ...
    - For container builds, `setup.sh` can detect CUDA from the toolkit path (`CUDA_HOME`, `nvcc`, or `/usr/local/cuda`) even when `nvidia-smi` is not available during `docker build`.
